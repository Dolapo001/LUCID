        ...
        {activeTab === 'dashboard' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <div>
                <h2 className="page-title">Herd Overview</h2>
                <p className="page-subtitle">Real-time status of monitored dairy cows</p>
              </div>
              <button className="btn btn-outline btn-sm" onClick={fetchBackendData}>🔄 Refresh Data</button>
            </div>

            {/* Dashboard Stats */}
            <div className="stats-grid">
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: 600 }}>Total Cows</span>
                <span style={{ fontSize: '2.25rem', fontWeight: 800, fontFamily: 'Space Grotesk' }}>{cows.length}</span>
                <span style={{ color: 'var(--status-healthy)', fontSize: '0.75rem' }}>● 100% Ingested</span>
              </div>
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: 600 }}>Active Alerts</span>
                <span style={{ fontSize: '2.25rem', fontWeight: 800, fontFamily: 'Space Grotesk', color: 'var(--status-mastitis)' }}>
                  {alerts.filter(a => a.status === 'new').length}
                </span>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>Needs immediate action</span>
              </div>
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: 600 }}>Herd Health Ratio</span>
                <span style={{ fontSize: '2.25rem', fontWeight: 800, fontFamily: 'Space Grotesk', color: 'var(--status-healthy)' }}>
                  {Math.round(((cows.length - alerts.filter(a => a.status === 'new' && a.trait !== 'estrus').length) / cows.length) * 100)}%
                </span>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>Excluding reproductive cycles</span>
              </div>
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: 600 }}>Avg Environment THI</span>
                <span style={{ fontSize: '2.25rem', fontWeight: 800, fontFamily: 'Space Grotesk', color: 'var(--status-heatstress)' }}>78.2</span>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>Tropical heat index load</span>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '2rem', marginTop: '2rem' }}>
              ...
              <div className="card" style={{ padding: '1.25rem' }}>
                <h3 style={{ marginBottom: '1rem', fontSize: '1.1rem', fontWeight: 700 }}>Animal Directory</h3>
                  ...
                  <table className="custom-table">
                    ...
                    <tbody>
                      {cows.map(cow => (
                        <tr 
                          key={cow.cow_id} 
                          style={{ cursor: 'pointer', backgroundColor: selectedCow?.cow_id === cow.cow_id ? 'rgba(99, 102, 241, 0.08)' : 'transparent' }}
                          onClick={() => setSelectedCow(cow)}
                        >
                          <td style={{ fontWeight: 700 }}>{cow.cow_id}</td>
                          <td>{cow.breed}</td>
                          <td>{cow.herd_id}</td>
                          <td>
                          ...
                          <td style={{ color: cow.temp > 39.0 ? 'var(--status-mastitis)' : 'inherit' }}>{cow.temp} °C</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {/* Cow Details and Historical Chart Drawer */}
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                {selectedCow ? (
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                      <h3 style={{ fontSize: '1.2rem', fontWeight: 700 }}>Cow Profile: {selectedCow.cow_id}</h3>
                      <span className={`badge ${
                        selectedCow.status === 'Healthy' ? 'badge-healthy' :
                        selectedCow.status === 'In-Estrus' ? 'badge-estrus' :
                        selectedCow.status === 'Mastitis Risk' ? 'badge-mastitis' :
                        selectedCow.status === 'Heat-Stressed' ? 'badge-heatstress' :
                        'badge-calving'
                      }`}>
                        {selectedCow.status}
                      </span>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem', fontSize: '0.875rem' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.02)', padding: '0.75rem', borderRadius: '10px' }}>
                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>Breed</span>
                        <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{selectedCow.breed}</span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.02)', padding: '0.75rem', borderRadius: '10px' }}>
                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>Herd identifier</span>
                        <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{selectedCow.herd_id}</span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.02)', padding: '0.75rem', borderRadius: '10px' }}>
                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>Activity Index</span>
                        <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{selectedCow.activity}</span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.02)', padding: '0.75rem', borderRadius: '10px' }}>
                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>Rumination Min/15min</span>
                        <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{selectedCow.rumination} m</span>
                      </div>
                    </div>

                    {/* Interactive SVG Trend Chart */}
                    <div style={{ marginTop: '1rem' }}>
                      <h4 style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '0.75rem', fontWeight: 600 }}>24-Hour Behavioural Trends</h4>
                      
                      <div style={{ background: '#0a0f1d', borderRadius: '12px', padding: '1rem', position: 'relative' }}>
                        ...
                        <svg viewBox="0 0 400 150" width="100%" height="150" style={{ overflow: 'visible' }}>
                          ...
                          {/* X-axis time points */}
                          <text x="30" y="140" fill="var(--text-secondary)" fontSize="8">00:00</text>
                          <text x="120" y="140" fill="var(--text-secondary)" fontSize="8">06:00</text>
                          <text x="210" y="140" fill="var(--text-secondary)" fontSize="8">12:00</text>
                          <text x="300" y="140" fill="var(--text-secondary)" fontSize="8">18:00</text>
                          <text x="370" y="140" fill="var(--text-secondary)" fontSize="8">24:00</text>
                          
                          {/* Y-axis labels */}
                          <text x="10" y="23" fill="var(--text-secondary)" fontSize="8">High</text>
                          <text x="10" y="73" fill="var(--text-secondary)" fontSize="8">Norm</text>
                          <text x="10" y="123" fill="var(--text-secondary)" fontSize="8">Low</text>
                          
                          {/* Trend line for Activity (Violet) */}
                          <path 
                            d={selectedCow.status === 'In-Estrus' 
                              ? "M 30,110 L 90,105 L 150,90 L 210,30 L 270,25 L 330,45 L 380,50" 
                              : selectedCow.status === 'Mastitis Risk'
                              ? "M 30,70 L 90,80 L 150,85 L 210,110 L 270,120 L 330,115 L 380,110"
                              : "M 30,75 L 90,65 L 150,80 L 210,70 L 270,68 L 330,72 L 380,69"
                            } 
                            fill="none" 
                          ...
                          {/* Trend line for Temperature (Pink/Red) */}
                          <path 
                            d={selectedCow.status === 'Mastitis Risk' 
                              ? "M 30,80 L 90,70 L 150,55 L 210,40 L 270,25 L 330,22 L 380,20" 
                              : selectedCow.status === 'Heat-Stressed'
                              ? "M 30,70 L 90,65 L 150,50 L 210,35 L 270,30 L 330,35 L 380,45"
                              : "M 30,85 L 90,82 L 150,80 L 210,78 L 270,83 L 330,86 L 380,84"
                            } 
                            fill="none" 
                            stroke="var(--accent-secondary)" 
                            strokeWidth="2"
                            strokeDasharray={selectedCow.status === 'Healthy' ? "3,3" : "0"}
                            strokeLinecap="round"
                          />
                        </svg>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', marginTop: '0.75rem', justifyContent: 'center', fontSize: '0.75rem' }}>
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                            <span style={{ display: 'inline-block', width: '12px', height: '4px', background: 'var(--accent-primary)' }}></span>
                            Activity Index
                          </span>
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                            <span style={{ display: 'inline-block', width: '12px', height: '4px', background: 'var(--accent-secondary)' }}></span>
                            Body Temperature
                          </span>
                        </div>
                      </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--text-secondary)' }}>
                    Select a cow from the list to inspect details.
                  </div>
                )}
              </div>
            </div>
